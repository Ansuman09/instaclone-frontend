...
const UserPosts=()=>{
    ...
    // const [oldPostsData,setOldPostsData]
    ...
    if (loading===true)
        ...
        return(
            ...
            <div >
                {posts.map((post,index)=>{
                  ...
                  const postsDataWithImageUrl = await Promise.all(
                    posts.map(async (post)=>{
                      const imageUrl=await fetchImageUrl(post.image_url);
                      return {
                        ...post,
                        imageUrl
                      }
                ...
                {/* <p onClick={}></p> */}
                <p onClick={()=>handleGetImage("1726882607754_1364318.jpeg")}>this is image</p>
                 {displayImageData && <img src={displayImageData}></img>}           
            </div>
            </div>    
        )
}
